---
title: "EC349: Week 5_Lecture 1_R Notebook_SOLUTION"
...
![](C:/Users/u2271250/OneDrive - University of Warwick/Warwick Economics/Teaching/EC349 Data Science/R Materials/Week 5 Lecture 2/Week_5_Lecture_2/Rolling_Sales_DataSet/Example_Brooklyn_Excel_File.jpg)
...
```{r}
setwd("C:/Users/u2271250/OneDrive - University of Warwick/Warwick Economics/Teaching/EC349 Data Science/R Materials/Week 5 Lecture 2/Week_5_Lecture_2")
```
...
```{r}
brooklyn_csv <- read.csv("/Users/gare.mac/Desktop/Warwick/Y3/EC349/Week 5 Lecture/Lecture 2/Rolling Sales DataSet/rollingsales_brooklyn_CSV.csv", skip = 4)
glimpse(brooklyn_csv)
```


This data is messier! [Beware that the mess in our data is different from the one in the online document. We see that some of the problems in the older dataset has been addressed - make your own comparison] And here’s how our CSV data is messier:

+ An addressed problem: In the older dataset (online document), the Character (string) data, such as <span style="color:magenta">ADDRESS</span>, was stored as class “factor”. Think of factors as categories or buckets. This is corrected in this more recent dataset.

+ <span style="color:magenta">GROSS.SQUARE.FEET</span> and <span style="color:magenta">SALE.PRICE</span> are also stored as character (strings) We can’t perform arithmetic operations, like calculating the mean, on a character!

+ <span style="color:magenta">SALE.DATE</span> is not stored in a format that represents calendar dates and times (e.g., <span style="color:magenta">dttm</span>). So we can’t build the histogram we saw above. (We can make a histogram, but it’s messy, and it makes no sense).

+ The <span style="color:magenta">GROSS.SQUARE.FEET, LAND.SQUARE.FEET</span>, and <span style="color:magenta">SALE.PRICE</span> columns contain a special character, the comma (,). They should be numeric (e.g., <span style="color:magenta">dbl</span>).
...
But if we load the same dataset with the <span style="color:magenta">read_csv()</span> [function](https://readr.tidyverse.org/reference/read_delim.html) from the readr package, which is part of the tidyverse, we see results ***similar***(spot the difference! Hint: Date) to our original approach with <span style="color:magenta">readxl()</span>:

brooklyn_csv <- read_csv("Rolling_Sales_DataSet/rollingsales_brooklyn_CSV.csv", skip = 4)
